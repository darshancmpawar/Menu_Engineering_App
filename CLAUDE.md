...
| `ikigai_masala-main/data/configs/city_rules/<city>.json` | **per-city rulesets** (one file per city: `bangalore.json` = the reference ruleset, `pune.json` = a standalone transcription of the Pune rulebook — see `docs/pune_rulebook.md`; `chennai.json` = standalone but NOT a rulebook transcription (none was supplied — it is the engine skeleton plus caps provable from the item list, see `docs/chennai_client_logic.md`); `hyderabad`/`ncr` still extend Bangalore). A file may `"extends": "<city>"` to inherit + override by rule `name` (and `"disable": [names]`). Loaded via `MenuRuleLoader.load_for_city(city)`; `CITY_RULES_DIR` + `DEFAULT_CITY='bangalore'` (the fallback for any city without its own file) live in `menu_rule_loader.py`. The API caches one ruleset per client's `city` |
| `ikigai_masala-main/data/configs/client_rules.json` | per-client overrides keyed by client name: `{disable, rules, constant_items}` (or legacy rule list); merged by `MenuRuleLoader.load_for_client()`. Keys starting with `_` are documentation everywhere, including inside `constant_items`. Bangalore clients are mapped in `docs/client_logics.md`, Pune's in `docs/pune_client_logic.md` |
...
| `ikigai_masala-main/scripts/remove_generic_rows.py` | drops 11 rows named for a *category* not a dish (`sweet`/`veg_gravy`/`salad`/`chutney`/… — Chennai 9, Pune 2). A menu printing "Sweet" is useless and no colour/ingredient/variety rule can reason about an unidentifiable dish. Client chose removal over renaming (D3); `dry_sweet`/`sweet` were on live ToastTab menus, so those sample rows are now unreproducible — a deliberate trade. None starves a required slot (smallest affected pool is Chennai soup, 8). Idempotent; `test_generic_rows.py` fails if any reappear |
