...
| Path | Purpose |
|---|---|
| `ikigai_masala-main/data/raw/city_items/<city>.xlsx` | **per-city item ontologies** — one file per city slug, selected from `clients.city`. `bangalore.xlsx` is the master **rule-ready** ontology (4,321 items; item_id key; cols incl. course_type, sub_category, cuisine_family, item_color, key_ingredient, primary_protein, is_premium_veg/_veg_dry, is_chinese_*/is_continental_*/is_*_biryani, is_liquid_dessert, welcome-drink subtypes (is_lassi/is_milkshake/is_soda_drink/is_cooler_drink/… + drink_rule_group), is_lentil_based/is_whole_legume_based/is_legume_salad, is_pulao, is_grill, is_dosa(_family), is_oil_based_bread, is_black_chana_gravy/is_kabuli_chana_gravy, plus quality signals classification_confidence/is_rule_ready/rule_data_note) and the fallback for any city without its own file. `pune.xlsx` = 274 items, all-veg, same 133 columns. `chennai.xlsx` = 361 items, same 133 columns, **has non-veg** (63 rows incl. 8 fish filed under `chicken_*` for want of a seafood bucket in the master) and **no welcome drinks at all**. Resolution + caching: `api/config.py::city_excel_path` / `city_required_slots`, `api/app.py::_get_menu_data(city)` |
