...
| `soft_preference_rule.py` | CP-SAT soft | **generic Phase-4 soft rule type** (objective-only, never INFEASIBLE): `mode` = `different_day` (penalise two selectors same day — premiums apart, #3), `avoid_consecutive` (penalise selector on adjacent days — regional nonveg/rice, #14-17), `avoid_attribute_repeat` (penalise a slot repeating an attribute value — key-ingredient variety, #1). `weight` sits below theme (~1e6, theme wins) and above the random tie-break (~1e3/cell). True lexicographic tiering is the deferred proper fix |
