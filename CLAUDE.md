...
| POST | `/api/v1/pool-preview` | F5: eligible distinct item count + category-wise counts for a set of `source_pools` (common always included) |
...
| `client_fixtures.py` | snapshot of the live `clients` table — **all 42 clients / 56 counters** (`CLIENTS`) + `APP_SETTINGS`. Regenerate when the live table changes shape |
...
56-counter sweep and the constraint-integration solves are **deselected by
