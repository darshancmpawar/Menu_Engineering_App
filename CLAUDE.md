...
9g. **A composition component can be impossible over the *horizon* even when every single day looks fine**: `apply()` capped each component to per-day candidate count, which misses uniqueness. L&T's 5-dish non-veg station has a kebab candidate every day but only ONE distinct kebab in its pool, so "a kebab daily" over five days is unsatisfiable however the solver picks. `_horizon_limited_components()` counts distinct matching items across the horizon and, when they fall short, `_add_horizon_floors()` replaces the per-day mandate with an at-least-N-days floor (N = what the pool can supply) using `OnlyEnforceIf` day indicators. Same bug class as the `selector_frequency` distinct-items cap (note 9c), one rule type later.
