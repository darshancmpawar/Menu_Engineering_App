...
| `ikigai_masala-main/data/raw/source_workbooks/` | **every client-supplied workbook in one place** — raw item lists, the Pune rulebook, Chennai's sample menu, Bangalore's sample history. Inputs, not runtime data: nothing in the app reads this directory, the normaliser reads *from* it to produce `city_items/<city>.xlsx`. Committed because the derived artefacts cite them, and without the sources a rule's reasoning is unverifiable. Only Bangalore's samples were committed before; Pune's and Chennai's raw lists, the rulebook and the sample menu existed solely as chat attachments and would have been lost. See its README for the per-file table and the add-a-city steps |
