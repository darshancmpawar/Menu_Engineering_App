...
### 4.6 `src/` top-level
- `constants.py` — `BASE_SLOT_NAMES`, `CONST_SLOTS`, `DEFAULT_OFF_SLOTS` (selectable-but-off-by-default: `curd_rice` + combos), `COMBO_CATEGORIES` + `combo_minority_count` (dal_rasam/sambar_rasam day-split), `DISPLAY_SLOT_NAME` (rice→"Flavoured Rice", bread→"Indian Bread", curd_rice→"Curd Rice", dal_rasam→"Dal / Rasam").
