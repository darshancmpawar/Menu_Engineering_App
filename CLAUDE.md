...
| `ikigai_masala-main/scripts/chennai_course_corrections.py` | four Chennai rows filed under the wrong `course_type`, which decides **which slot pool a dish lands in** — so a misfile makes a dish servable in the wrong position, not merely mislabelled. Found by running ToastTab CHN's real DB row: Tuesday's `veg_gravy__1` came back `semiya_pal_payasam`, a dessert, beside a kuzhambu. `millet_payasam`/`semiya_pal_payasam` `veg_gravy`→`dessert` and `kalkandu_pongal`/`mapillai_samba_sweet_pongal` `rice`→`dessert` (the workbook already filed `semiya_payasam` and `rice_kheer` correctly, so it disagreed with itself). Sets `dessert_form` too, or `dessert_form_non_consecutive` would silently skip the new rows. Idempotent; re-run after any re-import |
