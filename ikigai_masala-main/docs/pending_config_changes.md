...
| 1.3 | **TCL** | replace `salad` with a second `veg_gravy` (`slot_counts.veg_gravy = 2`) | "in salad need to give only KUZHAMBU item" — all 13 Chennai kuzhambus are `course_type = veg_gravy`, so the client's kuzhambu row is a second gravy, not the salad slot | `tcl_second_gravy_is_a_kuzhambu` is gated on `min_slot_count: 2` and does nothing; the counter serves a salad where the client serves a kuzhambu. |
