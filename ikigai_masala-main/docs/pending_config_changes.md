...
| **Chennai kuzhambu is a SALAD, and five other counters now see it** | You said "kuzhambus are the dish which category should be in salad not veg gravy", so `scripts/chennai_client_pools.py` re-files Chennai's twelve kuzhambus `veg_gravy` -> `salad` (veg_gravy 53 -> 41, salad 35 -> 47). TCL's stated rule now works — its salad is a kuzhambu 5 days of 5. **But the pool is city-wide**, so the five other Chennai counters with a salad slot now draw kuzhambus too, and none of them asked for it: Gartner 3 days of 5, RNTBCI Full Non-Veg and North Combo 3 of 5, ToastTab CHN 3 of 5, Tekion CHN 1 of 5. Correct if a kuzhambu is simply what a Chennai salad row holds; wrong if only TCL serves it that way. Scoping it back is one city rule (`salad_is_not_a_kuzhambu`) plus one `disable` line in `tcl.json` — say the word. | `scripts/chennai_client_pools.py::KUZHAMBU_TO_SALAD` |
