...
|---|---|---|---|---|
| **`item_color`, all cities** | **51 blank of 9,118**, all Hyderabad | — | `MenuSolver._add_color_constraints` clamps the day's required distinct colours to the number PRESENT, so a blank colour quietly relaxes the rule rather than merely going unchecked | ✅ **closed by you** — the enriched workbooks answered it: Bangalore, Chennai, NCR and Pune are 0% blank. The 51 left are Quest's own Hyderabad additions, which have no enriched file; `Chain rules/reports/colours_to_confirm_by_family.csv` is down from 455 questions to **26**. NB the client's own `chef_review` sheet flags 720 Bangalore + 354 NCR of the supplied colours as low-confidence |
| Chennai `welcome_drink` | **0** | ~25 | TCL, World Bank and both ICON lunch counters declare the slot; three state a buttermilk rule | ✅ closed — `Chain rules/chennai_client_pools.py` imports 28 (10 buttermilks incl. `sambaram` and `tadka_neer_mor`, the dishes TCL's own grid names) |
...
| Chennai chicken gravies | 13 reachable | ~20 | World Bank serves one every day; `ThemeSlotFilterRule` hid the other 12 because they were tagged `continental` | ✅ closed — `Chain rules/chennai_cuisine_corrections.py`; six saved weeks now hold at 5/5 |
...
| Pune `veg_dry` leafy · Pune chaat starters · Chennai Chinese veg gravy | — | — | Corning Chakan, and a Chennai Chinese day | ✅ closed — `Chain rules/deepen_thin_pools.py` |
| **`is_paneer_fry`, all cities** | **0 rows** | — | Zscaler's "exactly one paneer fry a week" selects on it, so the rule had never constrained anything (`min` caps to what the pool can place, and an empty selector places nothing) | ✅ closed — `Chain rules/definitional_flags.py` now derives it, and its twin `is_paneer_gravy`, from `primary_protein` + the dish name |
...
python Chain rules/canonical_dish_spellings.py     # one dish, one spelling
python Chain rules/bread_form_flags.py             # is_plain_phulka_chapathi, both ways
python Chain rules/nonveg_structural_flags.py      # dry vs gravy, and chicken flags off non-chicken dishes
python Chain rules/chennai_client_pools.py         # kootu -> dal, welcome drinks, boiled egg, bone salna
python Chain rules/chennai_cuisine_corrections.py  # 31 Indian dishes wrongly tagged continental
python Chain rules/deepen_thin_pools.py            # the three pools a rule outgrew
python Chain rules/complete_ontology.py            # last of the learners
python Chain rules/definitional_flags.py           # is_liquid_dessert / is_buttermilk, both ways
python Chain rules/build_pool_token_map.py         # refresh the committed token map
python Chain rules/audit_course_types.py           # must exit 0
...
Also re-run `Chain rules/dump_client_fixtures.py --clients <clients export>` whenever
...
| **Bangalore has no plain `boiled_egg`** | F5 and Plan View both pin one and both get stamped text, because the closest Bangalore row is `boiled_egg_with_pepper_masala` — a different dish. Chennai gained a real `boiled_egg` row for World Bank and ICON; Bangalore did not. Add one? | `Chain rules/chennai_client_pools.py` is the shape to copy |
...
| **NCR: twelve vegetarian keemas filed as mutton** | `soya_keema`, `veg_keema`, `nutri_keema` and nine siblings sit in `nonveg_main` with `primary_protein: mutton`. Their wrongly-inherited chicken-gravy flags are cleared, but whether a soya keema belongs on a non-veg counter at all is your call, not a data fix. | `Chain rules/nonveg_structural_flags.py` |
| ~~**Premium flags**~~ | **DEFERRED by the client.** Applying their cost definition would take Bangalore's `is_premium_gravy` from 174 rows to 463, at which point "one premium veg gravy a week" stops meaning "the week's showcase dish" and starts meaning "paneer at most once a week". Left at the narrower flag. | `Chain rules/complete_ontology.py::APPLY_PREMIUM` stays `False` |
