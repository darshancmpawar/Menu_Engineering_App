...
| "protein source list (choley, rajma, kala chana, lobia, moong, masoor, urad, toor/chana dal, sprouts, soya, paneer, tofu, besan chilla, peanut chaat, khichdi, ghugni…) — one of them on the daily menu, in flavoured rice / veg gravy / veg dry / salad / dal" | DONE | `tekion_protein_source_daily` (and `stryker_protein_source_daily`): `selector_frequency` with `daily_min: 1`, `base_slot` = the five slots as a list, selector = `any_of` over 16 `key_ingredient` values. Selected on `key_ingredient` because the client specified it that way. Needed two engine additions — `daily_min` (per-day floor; `min` counts days across the horizon and so depends on horizon length) and `base_slot` accepting a list (the constraint spans slots). `Chain rules/protein_key_ingredients.py` first folds 7 variant spellings (`channa_dal`/`gram_dal`→`chana_dal`, `cottage_cheese`→`paneer`, `soppu_moong`→`green_moong`, `allesande`→`black_eyed_pea`, `kofta_made_from_lentil`→`toor_dal`) that named a listed protein under another name. Verified over 3 rolling weeks × 2 clients: all 30 days carry one. **Not servable — Bangalore has no such dish:** soya chaap, moong chilla. **Pesarattu** exists but is filed as `bread`, outside the five slots. |
| "not just in dal — at least 3 days (the non-chinese, non-biryani days) carry a protein from the list OUTSIDE the dal slot, and the chinese and biryani days too where it does not break rules" | DONE | `*_protein_outside_dal` (`selector_frequency`, `min: 3`, `base_slot` = rice/veg_gravy/veg_dry/salad — dal excluded) plus `*_protein_outside_dal_all_days` (`soft_preference` `mode: prefer_daily`, same slots, priority medium). The floor is hard and auto-capped; the remaining two days are a soft push, because every day *can* supply one today but mandating all five would trade a whole plan for one dish the moment another rule disagrees — which is the opposite of "without breaking rules". Needed a new soft mode, `prefer_daily` (penalise each day the selector is absent). Verified over 3 rolling weeks × 2 clients: **15 of 15 days** carried a non-dal protein, so the soft push reached all five days in practice — Tekion's chinese Tuesdays took chickpea salads, its biryani Fridays `bitter_peanut_palya` / `amritsari_chole`. |
